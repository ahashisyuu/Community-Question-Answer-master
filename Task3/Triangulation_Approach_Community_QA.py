...
class QASystem(object):
    def __init__(self, sess, pretrained_embedding, init_word_embed, args):
        self.sess = sess
        ...
        self.question_topic = tf.placeholder(tf.float32, [None, 9], name="q_topic")
        ...
        self.question_topic_1 = tf.placeholder(tf.float32, [None, 9], name="q1_topic")
        ...
        self.comment_topic_1 = tf.placeholder(tf.float32, [None, 9], name="c1_topic")
        ...
        self.question_topic_2 = tf.placeholder(tf.float32, [None, 9], name="q2_topic")
        ...
        self.comment_topic_2 = tf.placeholder(tf.float32, [None, 9], name="c2_topic")
        ...
        with tf.variable_scope('qa'):
            # embedding #
            self.question_embedding, self.question_1_embedding, self.comment_1_embedding, \
            self.question_2_embedding, self.comment_2_embedding = self.setup_embedding()
    ...
    def rnn_encoder(self, input, input_length, name=None, keep_prob=1.0):
    ...
    def cnn_encoder(self, input, input_length, name=None, keep_prob=1.0):
        re_input = tf.reshape(input, [-1, self.args.max_length, self.args.embed_dim, 1])
        with tf.variable_scope("{}_CNN".format(name)):
            ...
            for i, filter_size in enumerate(self.args.filter_sizes):
                with tf.variable_scope("{}_filter_{}".format(name, filter_size)):
                    ...
                    pooled = tf.nn.max_pool(conv, ksize=[1, self.args.max_length - filter_size + 1, 1, 1],
    ...
    def setup_system(self):
        ...
        question_state = self.encoder(self.question_embedding, self.question_length, name="q_encoder")
        question_state_1 = self.encoder(self.question_1_embedding, self.question_length_1, name="q1_encoder")
        comment_state_1 = self.encoder(self.comment_1_embedding, self.comment_length_1, name='c1_encoder')
        question_state_2 = self.encoder(self.question_2_embedding, self.question_length_2, name='q2_encoder')
        comment_state_2 = self.encoder(self.comment_2_embedding, self.comment_length_2, name='c2_encoder')
        ...
        # concat #
        output = tf.concat([hq_1_output, hq_2_output, hq_3_output], axis=1)
        W = tf.get_variable('W_concat', [3, 2])
    ...
    def setup_loss(self):
        ...
        # loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.logits,
        #                                                              labels=tf.cast(self.labels, tf.float32)))
        accuracy = tf.reduce_mean(tf.cast(tf.equal(self.pred, self.labels), tf.float32))
        return loss, accuracy

    def train(self, q, q_pos, q_l, q_topic, q1, q1_pos, q1_l, q1_topic, c1, c1_pos, c1_l, c1_topic,
              q2, q2_pos, q2_l, q2_topic, c2, c2_pos, c2_l, c2_topic, label, keep_prob):
        feed_dict = {self.question: q, self.question_pos: q_pos, self.question_length: q_l,
                     self.question_topic: q_topic,
                     self.question_1: q1, self.question_pos_1: q1_pos, self.question_length_1: q1_l,
                     self.question_topic_1: q1_topic,
                     self.comment_1: c1, self.comment_pos_1: c1_pos, self.comment_length_1: c1_l,
                     self.comment_topic_1: c1_topic,
                     self.question_2: q2, self.question_pos_2: q2_pos, self.question_length_2: q2_l,
                     self.question_topic_2: q2_topic,
                     self.comment_2: c2, self.comment_pos_2: c2_pos, self.comment_length_2: c2_l,
                     self.comment_topic_2: c2_topic,
    ...
    def test(self, q, q_pos, q_l, q_topic, q1, q1_pos, q1_l, q1_topic, c1, c1_pos, c1_l, c1_topic,
             q2, q2_pos, q2_l, q2_topic, c2, c2_pos, c2_l, c2_topic, label, keep_prob):
