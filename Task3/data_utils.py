...
def creat_vocab(args):
    if os.path.exists(args.word_vocab_file) and os.path.exists(args.pos_vocab_file):
        word_vocab_list = load_vocab_pos_file(args.word_vocab_file)
        pos_vocab_list = load_vocab_pos_file(args.pos_vocab_file)
    ...
    return word_vocab_list, pos_vocab_list
...
def padding(token, pos, word2id, pos2id, length):
    result_token = []
    result_pos = []
    count = 0
    ...
    return result_token, result_pos, l


def make_pading(datasets_pos, word2id, pos2id, max_length, max_topic_length):
    train_datasets = []

    for k in range(len(datasets_pos)):
        question_topic, question_topic_pos, question_topic_length = padding(datasets_pos[k][0],
                                                                            datasets_pos[k][1],
                                                                            word2id, pos2id, max_topic_length)
        question, question_pos, question_length = padding(datasets_pos[k][2],
                                                          datasets_pos[k][3],
                                                          word2id, pos2id, max_length)

        question_1_topic, question_1_topic_pos, question_1_topic_length = padding(datasets_pos[k][4],
                                                                                  datasets_pos[k][5],
                                                                                  word2id, pos2id, max_topic_length)
        question_1, question_1_pos, question_1_length = padding(datasets_pos[k][6],
                                                                datasets_pos[k][7],
                                                                word2id, pos2id, max_length)
        comment_1, comment_1_pos, comment_1_length = padding(datasets_pos[k][8],
                                                             datasets_pos[k][9],
                                                             word2id, pos2id, max_length)

        question_2_topic, question_2_topic_pos, question_2_topic_length = padding(datasets_pos[k][10],
                                                                                  datasets_pos[k][11],
                                                                                  word2id, pos2id, max_topic_length)
        question_2, question_2_pos, question_2_length = padding(datasets_pos[k][12],
                                                                datasets_pos[k][13],
                                                                word2id, pos2id, max_length)
        comment_2, comment_2_pos, comment_2_length = padding(datasets_pos[k][14],
                                                             datasets_pos[k][15],
                                                             word2id, pos2id, max_length)

        local = [question_topic, question_topic_pos, question_topic_length,
                 question, question_pos, question_length,
                 question_1_topic, question_1_topic_pos, question_1_topic_length,
                 question_1, question_1_pos, question_1_length,
                 comment_1, comment_1_pos, comment_1_length,
                 question_2_topic, question_2_topic_pos, question_2_topic_length,
                 question_2, question_2_pos, question_2_length,
                 comment_2, comment_2_pos, comment_2_length,
                 datasets_pos[k][16]]
...
def make_test_padding(datasets_pos, word2id, pos2id, max_length, max_topic_length):
    test_datasets = {}
    count_data = 0
    count_key = 0
    ...
    for key in datasets_pos.keys():
        ...
        for k in range(len(datasets_pos[key])):
            count_data += 1
            question_topic, question_topic_pos, question_topic_length = padding(datasets_pos[key][k][0],
                                                                                datasets_pos[key][k][1],
                                                                                word2id, pos2id, max_topic_length)
            question, question_pos, question_length = padding(datasets_pos[key][k][2],
                                                              datasets_pos[key][k][3],
                                                              word2id, pos2id, max_length)

            question_1_topic, question_1_topic_pos, question_1_topic_length = padding(datasets_pos[key][k][4],
                                                                                      datasets_pos[key][k][5],
                                                                                      word2id, pos2id, max_topic_length)
            question_1, question_1_pos, question_1_length = padding(datasets_pos[key][k][6],
                                                                    datasets_pos[key][k][7],
                                                                    word2id, pos2id, max_length)
            comment_1, comment_1_pos, comment_1_length = padding(datasets_pos[key][k][8],
...
def batch_iter(train_datasets, batch_size, max_length, shuffle=True):
