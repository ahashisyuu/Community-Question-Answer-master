...
def load(path):
    ...
    print('laod data size:', input_size, '\n')
...
def save_h5(old_path, new_path, word2id, ids2id, max_length):
    print('save data from ', old_path, 'to the ', new_path)
    f = h5py.File(new_path, 'w')
    question, q_id, q_mask = [], [], []
    question_o, q_o_id, q_o_mask = [], [], []
    context, c_mask = [], []
    label = []
...
if __name__ == '__main__':
    ...
    for i in range(len(task_path)):
        ...
        save_h5(task_path[i], h5py_path[i], word2id, ids2id, config.max_length)
