...
class baseline(nn.Module):
    ...
    def __init__(self, vocab_size, pos_size, config):
        ...
        self.lookup_p = nn.Embedding(self.pos_size, self.embedding_size)

        self.q_size = self.embedding_size * 2
        self.q_o_size = self.embedding_size * 2 + self.q_h_size + self.c_h_size
        self.c_size = self.embedding_size * 2 + self.q_h_size

        self.q_lstm = nn.LSTM(self.q_size, self.q_h_size, self.num_layer, dropout=self.dropout)
        ...
        self.q_linear = nn.Linear(self.q_h_size, 1)
        self.q_o_linear = nn.Linear(self.q_o_h_size, self.tag_size)
        self.c_linear = nn.Linear(self.c_h_size, 1)
        self.c_t_linear = nn.Linear(self.c_h_size, self.tag_size)

        self.c_norm = nn.BatchNorm1d(self.max_length, self.c_h_size)
    ...
    def q_attention(self, x, x_mask):
        ...
        s = self.q_linear(x_flat).view(x.size(0), x.size(1))
        s.data.masked_fill_(x_mask.data, -float('inf'))
        w = F.softmax(s, 1)
        out = w.unsqueeze(1).bmm(x).squeeze(1)

        return out

    def c_attention(self, x, x_mask):
        x_flat = x.view(-1, x.size(-1))
        s = self.c_linear(x_flat).view(x.size(0), x.size(1))
        s.data.masked_fill_(x_mask.data, -float('inf'))
        w = F.softmax(s, 1)
        out = w.unsqueeze(1).bmm(x).squeeze(1)
    ...
    def question_lstm(self, q, q_p_id, q_mask):
        batch_size = q.size()[0]
        q_embed = self.lookup(q)
        q_p_embed = self.lookup_p(q_p_id)
        q_embed = torch.cat([q_embed, q_p_embed], -1)
    ...
    def context_lstm(self, c, q_out, c_p_id, c_mask):
        batch_size = c.size()[0]
        c_embed = self.lookup(c)
        c_p_embed = self.lookup_p(c_p_id)
        c_embed = torch.cat([c_embed, c_p_embed], -1)

        q_output = q_out.expand(self.max_length, *q_out.size())
    ...
    def question_o_lstm(self, q_o, q_out, c_out, q_o_p_id, q_o_mask):
        batch_size = q_o.size()[0]
        q_o_embed = self.lookup(q_o)
        q_o_p_embed = self.lookup_p(q_o_p_id)
        q_o_embed = torch.cat([q_o_embed, q_o_p_embed], -1)
        ...
        c_output = c_out.expand(self.max_length, *c_out.size())
    ...
    def get_lstm(self, q, q_o, c, q_p_id, q_o_p_id, c_p_id, q_mask, q_o_mask, c_mask):
        q_output = self.question_lstm(q, q_p_id, q_mask)
        # print('q-output-size :', q_output.size())
        c_output = self.context_lstm(c, q_output, c_p_id, c_mask)
        # print('c-output-size: ', c_output.size())
        q_o_output = self.question_o_lstm(q_o, q_output, c_output, q_o_p_id, q_o_mask)
        # print('q-o-output-size: ', q_o_output.size())
        q_o_output = self.norm(q_o_output)
        # print('q-o-output-size: ', q_o_output.size())
        return q_o_output

    def forward(self, q, q_o, c, q_p_id, q_o_p_id, c_p_id, q_mask, q_o_mask, c_mask):
        lstm = self.get_lstm(q, q_o, c, q_p_id, q_o_p_id, c_p_id, q_mask, q_o_mask, c_mask)
    ...
    # (q, q_id, q_p_id, q_mask, q_o, q_o_id, q_o_p_id, q_o_mask, c, c_p_id, c_mask, label)

    def get_tag(self, q, q_id, q_p_id, q_mask, q_o, q_o_id, q_o_p_id, q_o_mask, c, c_p_id, c_mask, label):
        s = self.forward(q, q_o, c, q_p_id, q_o_p_id, c_p_id, q_mask, q_o_mask, c_mask)
        s, t = torch.max(s, dim=-1)
    ...
    def get_loss(self, q, q_id, q_p_id, q_mask, q_o, q_o_id, q_o_p_id, q_o_mask, c, c_p_id, c_mask, label):
        s = self.forward(q, q_o, c, q_p_id, q_o_p_id, c_p_id, q_mask, q_o_mask, c_mask)
    ...
    def get_loss_c(self, q, q_id, q_p_id, q_mask, q_o, q_o_id, q_o_p_id, q_o_mask, c, c_p_id, c_mask, label):
        q_output = self.question_lstm(q, q_p_id, q_mask)
        c_output = self.context_lstm(c, q_output, c_p_id, c_mask)

        c_out = c_output.expand(self.max_length, *c_output.size())
